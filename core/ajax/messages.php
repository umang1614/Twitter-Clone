...
if (isset($_POST['sendMessage']) && !empty($_POST['sendMessage'])) {
    ...
    $get_id = $_POST['get_id'];
    if (!empty($message)) {
        $getFromU->create('messages', array(
            'messageTo' => $get_id, 'messageFrom' => $user_id,
            'message' => $message, 'messageOn' => date('Y-m-d H:i:s')
        ));
    }
}
...
if (isset($_POST['showMessage']) && !empty($_POST['showMessage'])) {
    $user_id = @$_SESSION['user_id'];
    $messages = $getFromM->resentMessages($user_id);
?>
                            ...
                            <div class="people-message" data-user="<?php echo $message->user_id; ?>">
                                <div class="people-inner">
                                    ...
                                    <div class="name-right2">
                                        <span><a href="<?php echo BASE_URL . $message->username; ?>">
                                                <?php echo $message->screenName; ?></a></span><span>@<?php echo $message->username; ?></span>
                                    </div>
                                    <span>
